...
import { Filters } from '@/app/analytics/components/Filters';
import { AttendeesTable } from '@/app/analytics/components/AttendeesTable';
import { EmailMarketing } from '@/app/analytics/components/EmailMarketing';
...
export default function InlineAnalytics({ eventId, onBack }: Props) {
  ...
  const handleSendEmail = useCallback(async () => {
    ...
    try {
      ...
      setToast({ type: 'error', message: 'Upgrade to premium before sending emails to attendees.' });
    } finally {
      setLoading(false);
    }
    ...
    <div className="max-w-5xl mx-auto space-y-5 pb-10">
      ...
      <div className="flex items-start justify-between gap-4 pt-1">
        <div className="min-w-0">
          <button
            onClick={onBack}
            className="flex items-center gap-1.5 text-sm text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white mb-3 transition-colors"
          >
            <BiArrowBack size={16} />
            Back to Events
          </button>
          <h1 className="text-xl font-bold text-gray-900 dark:text-white truncate">{event.title}</h1>
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">Analytics overview</p>
        </div>
        <button
          onClick={handleShare}
          className="flex-shrink-0 flex items-center gap-1.5 px-3 py-2 text-sm font-medium text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-lg transition mt-8"
        >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <StatCard label="Tickets sold" value={totalSold} icon={TicketIcon} accent="bg-[#f54502]/10 text-[#f54502]" />
        <StatCard label="Revenue" value={fmt(totalRevenue)} icon={CircleDollarSign} accent="bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400" />
        <StatCard label="Checked in" value={checkedIn} icon={ScanLine} accent="bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400" />
        <StatCard label="Remaining" value={remaining > 0 ? remaining : 0} icon={Users} accent="bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400" />
      </div>
                ...
                <div key={t.name} className="px-5 py-3 flex items-center gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="h-1.5 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
                      <div className="h-full bg-[#f54502] rounded-full transition-all" style={{ width: `${pct}%` }} />
                    </div>
                  </div>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5 sm:p-6">
        <div className="flex flex-col sm:flex-row sm:items-center gap-4">
          <div className="flex-1 min-w-0">
            <p className="text-xs font-semibold text-[#f54502] uppercase tracking-wide mb-1">Premium — Early Access</p>
            <h3 className="text-base font-semibold text-gray-900 dark:text-white mb-1">Unlock advanced analytics</h3>
